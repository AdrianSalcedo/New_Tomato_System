...
class OptimalControlProblem(object):
    def __init__(self, t_0=0.0, t_f=140.0, dynamics_dim=8, control_dim=4,
        ...
        self.lambda_final = np.array([0.0, 0.8, 0.0, 0.1, 0.0, 0.1, 0.0, 0.0])
    ...
    def g(self, x_k, u_k):
        ...
        rhs_s_y_p = (- beta_y_p * s_y_p * (n_v_whole * i_v) + (r_y_1 + u_1) * l_y_p + (r_y_2 + u_2) * i_y_p+ (r_a + u_3 )* i_a_p - alpha * s_y_p) / n_p_whole
        rhs_s_a_p = (- beta_a_p * s_a_p * (n_v_whole * i_v) + alpha * s_y_p) / n_p_whole
        rhs_l_y_p = (beta_y_p * s_y_p * (n_v_whole * i_v) - b_y * l_y_p - (r_y_1 + u_1) * l_y_p) / n_p_whole
        rhs_l_a_p = (beta_a_p * s_a_p * (n_v_whole * i_v) - b_a * l_a_p) / n_p_whole
        rhs_i_y_p = (b_y * l_y_p - (r_y_2 + u_2 ) * i_y_p) / n_p_whole
        rhs_i_a_p = (b_a * l_a_p - (r_a + u_3 ) * i_a_p) / n_p_whole
        rhs_s_v = (- beta_y_v * s_v * i_y_p * n_p_whole - beta_a_v * s_v * i_a_p *  n_p_whole - (gamma + u_4) * s_v + (1-theta) * mu) / n_v_whole
        rhs_i_v = (beta_y_v * s_v * i_y_p *  n_p_whole + beta_a_v * s_v * i_a_p * n_p_whole - (gamma + u_4) * i_v + theta * mu) / n_v_whole
    ...
    def lambda_function(self, x_k, u_k, lambda_k):
        ...
        rhs_l_1 = alpha * (-lambda_1 + lambda_2) + beta_y_p * (i_v * n_v_whole) * (- lambda_1 + lambda_3)
        rhs_l_2 = beta_a_p * (i_v * n_v_whole) * (- lambda_2 + lambda_4)
        rhs_l_3 = A_1 + ( r_y_1 + u_1 ) * (-lambda_3 + lambda_1) + b_y * (- lambda_3 + lambda_5)
        rhs_l_4 = b_a * (- lambda_4 + lambda_6)
        rhs_l_5 = A_2 + (r_y_2 + u_2) * (- lambda_5 + lambda_1) + beta_y_v * (s_v / n_v_whole) * n_p_whole * (- lambda_7 + lambda_8)
        rhs_l_6 = A_3 + (r_a + u_3) * (- lambda_6 + lambda_1) + beta_a_v * (s_v / n_v_whole) * n_p_whole * (- lambda_7 + lambda_8)
        rhs_l_7 = (beta_y_v * (i_y_p * n_p_whole) + beta_a_v * (i_a_p * n_p_whole)) * (- lambda_7 + lambda_8) - (gamma + u_4) * lambda_7
        rhs_l_8 = A_4 + beta_y_p * (s_y_p / n_p_whole) * (- lambda_1 + lambda_3) + beta_a_p * (s_a_p / n_p_whole) * (- lambda_2 + lambda_4) - (gamma + u_4) * lambda_8
        #
        #
        #
    ...
    def optimality_condition(self, x_k, u_k, lambda_k, n_max):
        ...
        aux_4 = (lambda_7 * (s_v / n_v_whole ) + lambda_8 * (i_v / n_v_whole)) / (2 * c_4)
